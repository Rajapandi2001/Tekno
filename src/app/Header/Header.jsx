      ...
      <div className='flex justify-between items-center h-24 max-w-full mx-auto px-4 text-black shadow-md'>
        ...
        <ul className='hidden md:flex font-medium'>
          {navItems.map(item => (
            <li
              key={item.id}
              className='p-4  m-2 cursor-pointer duration-300 hover:text-green-500'
        ...
        <ul
          className={`${nav ? 'z-50 fixed' : 'fixed left-[-100%]'
        ...
        >
          ...
          {navItems.map(item => (
            <li
              ...
              onClick={() => goToPage(item.path)} // This will close the mobile menu
              role="menuitem"
              aria-label={item.text}
            >
              {item.icon}
